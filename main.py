...
async def handle_forwarded_from_bot(message: Message) -> None:
    ...
    if message.forward_from.is_bot:
        ...
        # Извлечение последней ссылки на канал из caption_entities
        last_channel_url = None
        if message.caption_entities:
            for entity in message.caption_entities:
                if entity.type == "text_link" and entity.url.startswith("https://t.me/"):
                    last_channel_url = entity.url

        if last_channel_url:
            # Получаем название канала
            channel_name = await fetch_channel_name(last_channel_url)
            async with async_session() as session:
                try:
                    session.add(Channel(
                        channel_id=bot_id,  # Используем ID бота как временный идентификатор
                        channel_url=last_channel_url,
                        channel_name=channel_name
                    ))
                    await session.commit()
                    logging.info(
                        f"Канал сохранён: {last_channel_url} ({channel_name})")
                    await message.reply(f"✅ Канал сохранён: {last_channel_url} ({channel_name})")
                    return  # Завершаем обработку после успешного сохранения
                except Exception as e:
                    logging.error(f"Ошибка при сохранении канала: {str(e)}")
                    await session.rollback()
                    await message.reply(f"❌ Ошибка: {str(e)}")

        # Если last_channel_url отсутствует, отправляем сообщение об ошибке
        else:
